...
class CasingThicknessDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray = None, transform:Callable = None, **kwargs):
        """
        ...
        Parameters:
            X (np.ndarray): Input data with shape (num_samples, h, w).
            y (np.ndarray, optional): Segmentation masks with shape (num_samples, h, w).
            transform (callable, optional): Transformations to apply to X.
